...
int check_log_info(Message msg, Message *back_msg, int sockfd)
{
	...
	{
		printf("密码正确！\n");
	  	if(strcmp(msg.password, result[6]) == 0)
		{
			//检查用户是否在线
			while(temp != NULL)
			{
			    if(strcmp(msg.user, temp->user) == 0)
				{
					sqlite3_close(db);
					printf("用户已登录!\n");
			...
			strcpy(new_client->id, result[4]);
			strcpy(new_client->user, result[5]);
			insert_new_client(&new_client);
			
			//回传用户的用户名和个性签名
			strcpy(back_msg->user, result[5]);      
			if(result[7] != NULL)
			{
				strcpy(back_msg->signature, result[7]); 
			}
...
//处理用户的登录操作
void login(Message msg, int sockfd)
{
	Message back_msg;
	...
	if(back_msg.action == LOGIN_OK)
	{
		...
		strcpy(active_user, back_msg.user);
		bzero(&back_msg, sizeof(back_msg));
		
		//将所有用户信息发给当前用户
		if(sqlite3_open("user_info.db", &db) != SQLITE_OK)
		{
	    	error("Send online user info sqlite3_open error!");
		}	
	
		if((rc = sqlite3_get_table(db, "select user, signature from user_info", &result, &row, &column, &errmsg)) != SQLITE_OK)
		{
	    	error("Send online user info sqlite3_get_table error!");
		}
		...
		for(i = 1; i <= row; i++)
		{
			...
			while(temp != NULL)
			{
				if(strcmp(temp->user, result[i * column]) == 0)
				{
			...
			if(strcmp(result[i * column], active_user) == 0)
			{
				printf("自己不发送\n");
				continue;
			}
			...
			strcpy(back_msg.user, result[i * column]);
			if(result[i * column + 1] != NULL)
			{
				strcpy(back_msg.signature, result[i * column + 1]);
			}
			printf("好友：%s\n", back_msg.user);
			printf("个性签名：%s\n", back_msg.signature);
			...
			if(send(sockfd, &back_msg, sizeof(back_msg), 0) == -1)
			{
	   			error("Send online user info error!");
			}
		}
		printf("所有用户信息已发送！\n");
		sqlite3_close(db);
		...
		while(temp != NULL)
		{
			...
			{
			    error("Send new user online msg error!");
			}
		...
		while(temp1 != NULL)
		{
		    if(strcmp(temp1->target, msg.user) == 0)
			{
				bzero(&back_msg, sizeof(back_msg));
				back_msg.action = OFFLINE_MSG;
			    strcpy(back_msg.user, temp1->user);
			    strcpy(back_msg.message, temp1->message);
			    strcpy(back_msg.time, temp1->time);
		...
		while(temp2 != NULL)
		{
		    if(strcmp(temp2->target, msg.user) == 0)
			{
				bzero(&back_msg, sizeof(back_msg));
				
				back_msg.action = OFFLINE_FILE;
			    strcpy(back_msg.user, temp2->user);
			    strcpy(back_msg.file_name, temp2->file_name);
			    strcpy(back_msg.message, temp2->file_content);
				
	 			if(send(sockfd, &back_msg, sizeof(back_msg), 0) == -1)
				{
				    error("Send new user online file error!");
				}
		...
		while(temp3 != NULL)
		{
		    if(strcmp(temp3->target, msg.user) == 0)
			{
				bzero(&back_msg, sizeof(back_msg));
				back_msg.action = OFFLINE_GROUP_MSG;
			    strcpy(back_msg.user, temp3->user);
			    strcpy(back_msg.message, temp3->message);
			    strcpy(back_msg.time, temp3->time);
				
	 			if(send(sockfd, &back_msg, sizeof(back_msg), 0) == -1)
				{
				    error("Send new user online group msg error!");
				}
