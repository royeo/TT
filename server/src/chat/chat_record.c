...
void handle_check_record(Message msg, int sockfd)
{
	...
	{
    	error("Check message record sqlite3_open error!");
	}	
	...
	{
	    error("Check message record sqlite3_get_table error!");
	}
	...
	for(i = 1; i <= row; i++)
	{
		...
		strcpy(back_msg.time, result[i * column]);
		strcpy(back_msg.message, result[i * column + 1]);
		
		printf("%s\n", back_msg.message);
		if(send(sockfd, &back_msg, sizeof(back_msg), 0) == -1)
		{
			error("Send message record error!");
		}
	}
	...
	if(send(sockfd, &back_msg, sizeof(back_msg), 0) == -1)
	{
	    error("Send message send error!");
	}
	sqlite3_close(db);
}

//用户请求删除好友聊天记录
void delete_chat_record(Message msg, int sockfd)
{
	char *errmsg = NULL;
	char sql[1024];
	sqlite3 *db;      
	...
	if(sqlite3_open("MsgEx.db", &db) != SQLITE_OK)
	{
    	error("Delete message record sqlite3_open error!");
	}	
	...
	{
		error("Delete message record sqlite3_exec error!");
	}
	...
	if(send(sockfd, &back_msg, sizeof(back_msg), 0) == -1)
	{
		error("Delete message record send error!");
	}
	sqlite3_close(db);
}
...
void save_message_record(Message msg)
{
	...
	if(sqlite3_open("MsgEx.db", &db) != SQLITE_OK)
	{
    	error("Save message record sqlite3_open error!");
	}	
	...
	if(sqlite3_exec(db, sql, NULL, NULL, &errmsg) != SQLITE_OK)
	{
		error("Save message record sqlite3_exec error!");
	}
...
void handle_check_group_record(Message msg, int sockfd)
{
	...
	if(sqlite3_open("MsgEx.db", &db) != SQLITE_OK)
	{
    	error("Check group message record sqlite3_open error!");
	}	
	...
	if((rc = sqlite3_get_table(db, sql, &result, &row, &column, &errmsg)) != SQLITE_OK)
	{
	    error("Check group message record sqlite3_get_table error!");
	}
	...
	if(send(sockfd, &back_msg, sizeof(back_msg), 0) == -1)
	{
	    error("Send group message send error!");
	}
...
void save_group_msg_record(Message msg)
{
	...
	if(sqlite3_open("MsgEx.db", &db) != SQLITE_OK)
	{
    	error("Save group message record sqlite3_open error!");
	}	
	...
	if(sqlite3_exec(db, sql, NULL, NULL, &errmsg) != SQLITE_OK)
	{
		error("Save group message record sqlite3_exec error!");
	}
