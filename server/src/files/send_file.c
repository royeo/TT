...
void send_file(Message msg, int sockfd)
{
	...
	while(temp != NULL)
	{
	    if(strcmp(temp->user, msg.target) == 0)
		{
			//用户在线,发送文件给目标客户端
			...
			strcpy(back_msg.file_name, msg.file_name);
	...
	/*	Offline_File *new_offline_file;
		...
		strcpy(new_offline_file->user, msg.user);
		strcpy(new_offline_file->target, msg.target);
		strcpy(new_offline_file->file_name, msg.file_name);
		strcpy(new_offline_file->file_content, msg.message);
